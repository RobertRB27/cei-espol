...
import { generateSolicitudPDF } from '@/lib/pdf/solicitud-template';
...
export async function GET(
  ...
  try {
    ...
    const pdfData = {
      ...
      title: metadata.title || 'Solicitud de Análisis',
      ...
      level: metadata.level || 'Inexistente',
      risk: metadata.risk || 'Exenta de riesgos',
    };
    
    // Generate the PDF
    const pdfBytes = await generateSolicitudPDF(pdfData);
    ...
    if (isDownload) {
      headers['Content-Disposition'] = `attachment; filename="Solicitud_${application.codification}.pdf"`;
    } else {
    ...
    console.error('Error generating solicitud PDF:', error);
